...
const List<String> _lunarDayNames = [
  '初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十',
  '十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九', '二十',
  '廿一', '廿二', '廿三', '廿四', '廿五', '廿六', '廿七', '廿八', '廿九', '三十',
];
int _lunarLeapMonth(int y) => _lunarInfo[y - 1900] & 0xf;
int _lunarLeapDays(int y) => _lunarLeapMonth(y) > 0
    ? ((_lunarInfo[y - 1900] & 0x10000) != 0 ? 30 : 29)
    : 0;
...
/// 返回指定公历日期的农历日名（如"初六"），超出 1900-2100 范围返回空串。
String _lunarDayOf(DateTime dt) {
  if (dt.year < 1900 || dt.year > 2100) return '';
  var offset = dt.difference(DateTime(1900, 1, 31)).inDays;
  if (offset < 0) return '';
  int i, temp = 0;
  ...
  var isLeap = false;
  int m;
  for (m = 1; m < 14 && offset > 0; m++) {
    if (leap > 0 && m == (leap + 1) && !isLeap) {
      m--;
      isLeap = true;
      temp = _lunarLeapDays(y);
    } else {
      temp = _lunarMonthDays(y, m);
    }
